...
reg <- nnet::multinom(REST_USE ~ DRIVER*AGE**2 + SEX + AGE**2, data = variables)
...
Pr_SEATBELT = length(which(variables$REST_USE == 7))/length(variables$REST_USE)

## Standard diff
propensity <- variables %>% mutate(P_SCORE = if_else(REST_USE == 7, reg, 1-reg)) %>%
  mutate(P_STAB = if_else(REST_USE == 7, Pr_SEATBELT/P_SCORE, (1-Pr_SEATBELT)/P_SCORE))

treatment <- propensity %>% filter(REST_USE == 7) 
control <- propensity %>% filter(REST_USE != 7)
